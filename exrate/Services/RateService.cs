...
namespace Exrate.Services
{
    public class RateService : IRateService
    {
        ...
        public bool UpdateKrskRates()
        {
            ...
            GetSberbankRates("http://sberbank.ru/krasnoyarsk/ru/");
            
            int banksUpd;
            if ((banksUpd = _rateRepository.SaveRates(_rates)) > 0)
            {
        ...
        private void GetUnitedBankRates()
        {
            ...
            try
            {
                ...
                var nodes = htmlPage.DocumentNode.SelectSingleNode("//table//table//tr[2]").ChildNodes;
                var dateStr = htmlPage.DocumentNode.SelectSingleNode("//table//table//tr").InnerText;
                var date = DateTime.Parse(new Regex(DATE_REGEX).Match(dateStr).ToString());
                var buyAndSell = nodes[2].InnerText;
                var tokens = buyAndSell.Split('/');
                var usd = new CurrencyModel
                {
                    ValueName = Currencies.USD,
                    Buy = Decimal.Parse(tokens[0].Remove(tokens[0].Length - 2, 2)),
                    Sell = Decimal.Parse(tokens[1].Remove(tokens[1].Length - 2, 2)),
                };
                nodes = htmlPage.DocumentNode.SelectSingleNode("//table//table//tr[3]").ChildNodes;
                buyAndSell = nodes[2].InnerText;
                tokens = buyAndSell.Split('/');
                var eur = new CurrencyModel
                {
                    ValueName = Currencies.EUR,
                    Buy = Decimal.Parse(tokens[0].Remove(tokens[0].Length - 2, 2)),
        ...
        private void GetSberbankRates(string url)
        {
            ...
            {
                var htmlPage = htmlWeb.Load(url);
                var nodes = htmlPage.DocumentNode.SelectSingleNode("//table[@class='table1_eggs4']//tr[2]").ChildNodes;
                var dateStr = htmlPage.DocumentNode.SelectSingleNode("//div[@class='eggs4_v']//div//span").InnerText;
                var date = DateTime.Parse(new Regex(DATE_REGEX).Match(dateStr).ToString());
                ...
                nodes = htmlPage.DocumentNode.SelectSingleNode("//table[@class='table1_eggs4']//tr[3]").ChildNodes;
